...
#[test]
fn test_exec_health_with_unreachable_http_endpoint() {
    let opamp_server = FakeServer::start_new();

    let local_dir = tempdir().expect("failed to create local temp dir");
    let remote_dir = tempdir().expect("failed to create remote temp dir");

    // Create agent type with executable that tries to connect to a non-existent health endpoint
    create_file(
        r#"
namespace: test
name: test
  ...
  on_host:
    health:
      ...
      http:
        ...
        port: 18003  # Use a port that's likely not in use
    executables:
      - id: test-exec
        path: /bin/ls
        args: /unknown/path
...
"#;
    ...
    // TODO: The UI only shows the binary error, but on the test we can actually see both errors
