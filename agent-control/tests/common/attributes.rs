...
use newrelic_agent_control::opamp::capabilities::CustomCapabilities;
...
pub fn check_custom_capabilities_match(
    ...
    let expected: HashSet<String> = expected.as_slice().iter().map(|c| c.to_string()).collect();
    if current != expected {
        return Err(format!(
