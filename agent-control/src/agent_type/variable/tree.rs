...
impl VariableTree {
    ...
    /// provided user values.
    ///
    /// Errors when a required variable has no user value, or when a user value doesn't match the
    /// declared type. User-config keys with no matching definition are logged as `WARN` and
    /// ignored.
...
fn resolve_sub_tree(
    ...
    for (key, subtree) in sub_tree.into_iter() {
        ...
        match subtree {
            VariableTreeNode::End(def) => match (user_value, def.default) {
                (Some(v), _) => {
                ...
                (None, None) => missing.push(partial_variable_path),
            },
