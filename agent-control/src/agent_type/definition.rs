...
/// The agent type as parsed from a YAML file. Variables are stored as a [`VariableTree`] and
/// resolved via [`VariableTree::resolve`] with the user-supplied values to obtain
/// fully-populated [`VariableValue`]s ready for rendering.
///
...
pub mod tests {
    ...
    impl AgentTypeDefinition {
        ...
        /// Resolves the AgentType's variables with the provided yaml values (helper for testing
        /// purposes).
        ///
        /// # Panics
        ///
    ...
    fn test_bad_parsing() {
        ...
        assert!(
            raw_agent_err
                .unwrap_err()
                .to_string()
                .contains("data did not match any variant of untagged enum VariableTreeNode")
        );
    }
