...
impl Registry {
    /// Builds a [Registry] whose local source loads the embedded agent types and overlays the
    /// custom agent types found in the given directory (custom definitions take precedence).
    pub fn new(config: RegistryConfig) -> Self {
        Self {
            local: LocalRegistry::new(config.custom_agent_types_path),
        }
    }
}
