...
impl LocalRegistry {
    /// Builds a [LocalRegistry] loading the embedded agent types and overlaying the custom agent
    /// types found in the given directory (custom definitions take precedence over the embedded
    /// ones).
    pub fn new(custom_agent_types_path: PathBuf) -> Self {
        let mut registry =
            Self::try_new(embedded_definitions()).expect("Conflicting agent type definitions");
...
pub mod tests {
    ...
    impl LocalRegistry {
        /// Builds a [LocalRegistry] with only the embedded agent types (no custom ones).
    ...
    fn test_default_loads_embedded_definitions() {
        ...
        let registry_nonexistent_custom = LocalRegistry::new(PathBuf::from("/nonexistent/path"));
        assert_eq!(
...
"#
        ...
        let registry = LocalRegistry::new(path.to_path_buf());

        // Custom over earlier custom: the second file's definition wins.
        let custom = registry
            .get(&AgentTypeID::try_from("ns/io.test:0.0.0").unwrap())
            .unwrap();
        assert!(!custom.variables.k8s.0.contains_key("version"));
        assert!(custom.variables.k8s.0.contains_key("different"));
