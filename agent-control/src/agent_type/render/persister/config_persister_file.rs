    ...
    W: FileWriter,
{
    ...
    fn write(&self, path: &Path, content: &str) -> Result<(), WriteError> {
        self.file_writer
            .write(path, content.to_string(), get_file_permissions())
    }
...
mod tests {
    ...
    fn test_persist_multiple_single_files() {
        ...
        directory_manager.should_create(generated_conf_path.as_path(), get_directory_permissions());
        directory_manager.should_create(agent_files_path.as_path(), get_directory_permissions());
    ...
    fn test_persist_multiple_single_map_files() {
        ...
        directory_manager.should_create(config_kafka_path.as_path(), get_directory_permissions());
        ...
        // We need to mock create because path will never exist on the test
        directory_manager.should_create(config_mysql_path.as_path(), get_directory_permissions());
        ...
        // We need to mock create because path will never exist on the test
        directory_manager.should_create(config_redis_path.as_path(), get_directory_permissions());
    ...
    fn test_writing_filemap_type_should_stop_processing_the_rest() {
        ...
        // We need to mock create because path will never exist on the test
        directory_manager.should_create(integrations_path.as_path(), get_directory_permissions());
        let mut integrations_path = agent_files_path.clone();
        integrations_path.push("integrations.d");
        // We need to mock create because path will never exist on the test
        directory_manager.should_create(integrations_path.as_path(), get_directory_permissions());
