...
trait VariablesOnHostExt {
    ...
    fn with_packages(self, packages: &RenderedPackages) -> Result<Variables, AgentTypeError>;
}

impl VariablesOnHostExt for Variables {
    fn with_packages(mut self, packages: &RenderedPackages) -> Result<Variables, AgentTypeError> {
        ...
        for (package_id, package) in packages {
            let attribute_name =
                compute_path_suffix(&package.download.oci.reference).map_err(|e| {
...
impl Templateable for OnHost {
    ...
    fn template_with(self, variables: &Variables) -> Result<Self::Output, AgentTypeError> {
        ...
        let extended_vars = variables.clone().with_packages(&rendered_packages)?;
