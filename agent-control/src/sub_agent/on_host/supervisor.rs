    ...
    PM: PackageManager,
{
    ...
    fn stop(self) -> Result<(), ThreadContextStopperError> {
        ...
        if let Err(err) = self.filesystem.delete_ephemeral(&LocalFile, &DirectoryManagerFs) {
    ...
    PM: PackageManager,
{
    ...
    fn spin_up(
        ...
        if let Err(err) = self.filesystem.delete_not_declared(&LocalFile, &DirectoryManagerFs) {
            warn!(?err, "filesystem: not declared entries cleanup failed on start");
        }
