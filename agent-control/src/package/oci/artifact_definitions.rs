...
/// Represents a downloaded OCI artifact with its complete metadata.
#[derive(Debug)]
...
/// Represents a downloaded OCI blob stored locally.
#[derive(Debug)]
...
/// Represents a Agent Package OCI artifact that has been downloaded.
...
pub mod tests {
    ...
    fn test_local_artifact_to_agent_package_success(
        ...
        // Act
        let result = LocalAgentPackage::try_from(artifact);

        // Assert
        assert!(
            result.is_ok(),
            "Expected successful conversion for case: {}",
            description
        );
    }
    ...
    fn test_local_artifact_to_agent_package_failure(
        ...
        assert!(
            ...
            "Expected error containing '{}', but got: '{}' for case: {}",
