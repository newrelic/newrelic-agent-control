    ...
    A: EffectiveAgentsAssembler,
{
    ...
    fn build(
        ...
        let mut supervisors: Option<Vec<SupervisorOnHost<command_supervisor::NotStarted>>> = None;
        ...
        if supervisors.is_none() {
            supervisors = Some(build_supervisors(effective_agent_res?, tx)?);
        }
