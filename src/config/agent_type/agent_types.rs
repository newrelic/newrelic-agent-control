    ...
    objects:
      cr1:
        ...
        spec:
          ...
          str: ${config.yaml_string}
          list: ${config.list}
"#;
    const K8S_CONFIG_YAML_VARIABLES: &str = r#"
...
"#;
    ...
    fn test_k8s_config_yaml_variables() {
        ...
        let user_config: AgentValues = serde_yaml::from_str(K8S_CONFIG_YAML_VARIABLES).unwrap();

        let expected_spec_yaml: &str = r#"
values:
  key: value
  another_key:
    nested: nested_value
...
list:
  - item1
  - item2
"#; // TODO: null values are not supported for now
        ...
        let spec = cr1.fields.get("spec").unwrap();
        println!("{:?}", spec);
        assert_eq!(expected_spec_value, spec.to_owned())
    }
}
