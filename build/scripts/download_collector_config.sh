...
TEMPLATE_DEFAULT="build/examples/values-nr-otel-collector-agent-linux-${AGENT_TYPE_VERSION}.yaml"
TEMPLATE_GATEWAY="build/examples/values-nr-otel-collector-gateway-${AGENT_TYPE_VERSION}.yaml"
...
docker run --rm -v "$PWD":/usr/src/app -w /usr/src/app ubuntu /bin/bash -c "sed -i -e '/${CONFIG_PLACEHOLDER}/{r ${BUILD_TMP_FOLDER}/${DEFAULT_CONFIG_FILENAME}' -e 'd}' ${TEMPLATE_DEFAULT}"
docker run --rm -v "$PWD":/usr/src/app -w /usr/src/app ubuntu /bin/bash -c "sed -i -e '/${CONFIG_PLACEHOLDER}/{r ${BUILD_TMP_FOLDER}/${GATEWAY_CONFIG_FILENAME}' -e 'd}' ${TEMPLATE_GATEWAY}"
