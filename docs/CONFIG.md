...
Agent control will load the configuration from the file `config.yaml` in the corresponding directory (`/etc/newrelic-agent-control/local-data/agent-control/` or `/opt/homebrew/var/lib/newrelic-agent-control`).
