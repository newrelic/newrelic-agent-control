...
- On-host: reject remote configs where two agents would claim the same `shared_filesystem` path (single-owner rule). The corresponding remote configuration is reported as `Failed`.
