...
- Add validation rejecting the `persistent` flag on `shared_filesystem` entries: shared entries are always persistent (removed only on uninstall, never on stop), so the flag has no effect and setting it (either `true` or `false`) is now an error.
