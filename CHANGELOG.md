...
- Self-instrumentation metrics now include a resource sampler reporting CPU/memory usage for Agent Control itself and its on-host sub-agents' child processes, plus counters/gauges for agent lifecycle, remote config, OpAMP connectivity, and HTTP status requests.
- Self-instrumentation traces now cover the on-host/Kubernetes supervisor lifecycle, remote-config processing, signature validation, and the HTTP status endpoint, in addition to startup. All agent-identity span fields are now consistently named `agent_id` (previously a mix of `id` and `agent_id`, which caused some spans to be silently dropped by dashboard widgets filtering on `agent_id`).
