use crate::common::{create_foo_crd, test_cr, Foo, FooSpec, K8sCluster, K8sEnv};
...
async fn k8s_delete_dynamic_resource() {
    ...
    test_cr(test.client.to_owned(), test_ns.as_str(), cr_name).await;

    let executor: K8sExecutor = K8sExecutor::try_default(test_ns.to_string()).await.unwrap();
    executor
        .delete_dynamic_object(GroupVersionKind::gvk("newrelic.com", "v1", "Foo"), cr_name)
        .await
        .unwrap();
