...
pub struct CloudIdDetector<AWS: Detect, AZURE: Detect> {
    aws_detector: AWS,
    azure_detector: AZURE,
}

impl Default for CloudIdDetector<AWSDetector<HttpClientUreq>, AzureDetector<HttpClientUreq>> {
    fn default() -> Self {
        Self {
...
impl<AWS, AZURE> Detect for CloudIdDetector<AWS, AZURE>
where
    AWS: Detect,
    AZURE: Detect,
{
...
mod test {
    ...
    impl<C> CloudIdDetector<AWSDetector<C>, AzureDetector<C>>
    where
        C: HttpClient,
    {
        fn new(aws: AWSDetector<C>, azure: AzureDetector<C>) -> Self {
            Self {
                aws_detector: aws,
                azure_detector: azure,
            }
