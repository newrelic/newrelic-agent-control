...
/// The local configuration for the super-agent is empty and a remote configuration including
/// a sub-agent is set (we check that the remote configuration is applied correctly).
/// After that, the super agent is rebooted using the same empty configuration. We check that the latest
