...
pub enum HealthCheckerError {
    ...
    Generic(String),
    #[cfg(feature = "k8s")]
    #[error("missing field `{0}` in k8s object `{1}` with name `{2}`")]
    MissingK8sObjectField(String, String, String),
    #[cfg(feature = "k8s")]
    #[error("invalid field `{0}` in k8s object `{1}`: {2}")]
    InvalidK8sObjectField(String, String, String),
    #[cfg(feature = "k8s")]
